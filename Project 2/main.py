...
def run_simulation(amount, ship_size, detection_radius, alpha, bot_number, saved_ship_layout_file = None, render_debug_logs = False):
    ...
    for i in range(amount):
        ...
        sim = Simulation(ship_size, detection_radius, alpha, bot_number, None, render_debug_logs)
...
for a in a_hyperparameters:
    result = run_simulation(
        amount=2, # number of simulations to run
        ship_size=20, # (D value)
        detection_radius=5, # (k value)
        alpha=a, # (a value)
        bot_number=9, # Bot1, Bot2, Bot3 or Bot4?
    ...
    print(f"Average Action (k={5}, a={a}, runs={10}): ", result, " per run")
